...
Koszyk jest lokalnym stanem; zamówienia wysylasz do OrderService z promoteami.
...
export function Checkout() {
  ...
  const placeOrderMutation = useMutation({
    ...
    onSuccess: (order) => {
      clearCart();
      // Redirect do order confirmation
...
- Zawsze generuj unique `idempotencyKey` (UUID v4) dla kazdego zamówienia.
- Wysylaj go w header `Idempotency-Key`.
- Backend bedzie deduplikowal duplikaty zamówień za pomoca tego klucza.

## Obsługa statusu zamówienia

- Status zmienia się asynchronicznie: `Created` -> `PaymentPending` -> `PaymentAuthorized` -> `Fulfilled`.
- Poling: co kilka sekund sprawdzaj status za pomoca `GET /api/orders/{orderId}`.
- WebSocket (opcjonalnie): nasłuchuj zmian statusu w real-time z serwera.
