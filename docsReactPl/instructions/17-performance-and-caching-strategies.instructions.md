...
Aplikacja musi być szybka i responsywna, oparta na efektywnym cache i load strategiach.

## Bundle Size Target

- **Main bundle**: < 150KB (gzipped).
- **React Query bundle**: ~ 30KB (gzipped).
- **UI library**: ~ 40KB (gzipped) - preferuj small/headless libraries.
...
export const queryClient = new QueryClient({
  defaultOptions: {
    ...
    mutations: {
      retry: 0, // Mutations nie retry'uj automatycznie
    },
  },
});
```

## Redis-Like Caching na Froncie
...
```typescript
// Używaj Next Image lub odpowiednika
<img
  src={productImage}
  alt={productName}
  width={400}
...
```typescript
// Memoizuj komponenty które się nie zmieniają
const ProductCard = memo(({ product }: { product: Product }) => {
  return <div>{product.name}</div>;
}, (prev, next) => prev.product.id === next.product.id);

// Używaj useCallback dla stabilnych referencji
...
// Unikaj inline objects/functions w props
...
Sprawdzaj w Lighthouse: `npm run build && npm run preview`.
