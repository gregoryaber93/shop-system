...
Testy integracyjne pokrywają rzeczywiste user flows z mockowanymi API endpoints.

## Test Setup z MSW (Mock Service Worker)
...
- **Unit tests**: Pojedyncze komponenty/hooki w izolacji (reducer logic, formatting).
- **Integration tests**: Flow cover: auth + data + mutations + UI updates.
- **Test real flows**, nie implementacyjne szczegóły.

## Pokrycie testami

- Krytyczne ścieżki: register, login, checkout, order status.
