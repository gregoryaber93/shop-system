...
Backend returns standard ProblemDetails (RFC 7807). Frontend must handle them.
...
interface ProblemDetails {
  ...
  correlationId?: string; // For server log tracing
  instance?: string; // Resource path
  extensions?: Record<string, any>; // Additional info
}
...
- **409/422**: Show validation error message; allow retry.
...
- Every request should have `X-Correlation-Id` header.
- Response contains `correlationId` in ProblemDetails.
- Display `correlationId` in UI on error for server log tracing.
